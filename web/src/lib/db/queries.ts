...
import { flightUserAccessCondition } from './flight-access';
...
export const listFlightBaseQuery = (db: Kysely<DB>, userId?: string) => {
  ...
  query = query.where(flightUserAccessCondition(userId));
...
export const createFlightPrimitive = async (
  ...
  return await db.transaction().execute(async (trx) => {
    const result = await createFlightPrimitiveWithConnection(
      trx,
      data,
      createdById,
    );
