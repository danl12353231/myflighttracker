...
import { flightUserAccessCondition } from '$lib/db/flight-access';
...
export const flightTrackRouter = router({
  list: authedProcedure
    ...
    .query(async ({ ctx: { user }, input }) => {
      ...
      if (scopedUserId) {
        query = query.where(flightUserAccessCondition(scopedUserId));
      }
  ...
  get: authedProcedure
    ...
    .query(async ({ ctx: { user }, input }) => {
      const flight = await db
        ...
        .$if(user.role === 'user', (query) =>
          query.where(flightUserAccessCondition(user.id)),
        )
        .executeTakeFirst();
