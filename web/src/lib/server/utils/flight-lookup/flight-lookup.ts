...
import { getFlightRoute as getFlightStatsFlightRoute } from './flightstats-provider';
...
const flightStatsProvider: FlightLookupProvider = {
  getFlightRoute: async (flightNumber, opts) => {
    try {
      return await getFlightStatsFlightRoute(flightNumber, opts);
    } catch (error) {
      console.warn(
        '[flight-lookup] FlightStats lookup failed; falling back to ADSBDB:',
        error,
      );
      return await getAdsbdbFlightRoute(flightNumber);
    }
  },
};
