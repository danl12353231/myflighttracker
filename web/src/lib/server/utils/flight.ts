...
export type UpcomingFlightScope = 'mine' | 'friends';

/**
 * Upcoming (not yet completed) flights where the given user is a passenger.
 * `scope: 'friends'` additionally requires at least one guest passenger on the
 * flight, i.e. flights recorded for friends without an account.
 */
export const listUpcomingFlights = async (
  userId: string,
  scope: UpcomingFlightScope = 'mine',
): Promise<Flight[]> => {
  let query = listFlightBaseQuery(db, userId);
    ...
    query = query.where((eb) =>
      eb.exists(
        eb
          ...
          .where('flightPassenger.guestName', 'is not', null),
      ),
    );
  }

  const flights = (await query.execute()) as Flight[];

  return flights
    .filter((flight) => !isCompletedFlight(flight))
    .sort((a, b) => {
      const aTime =
        resolveFlightTimeline(a).effectiveDeparture?.getTime() ??
        Number.MAX_SAFE_INTEGER;
      const bTime =
        resolveFlightTimeline(b).effectiveDeparture?.getTime() ??
        Number.MAX_SAFE_INTEGER;
      return aTime - bTime;
    });
};
...
export const createFlight = async (data: CreateFlight, createdById: string) => {
  return await createFlightPrimitive(db, data, createdById);
};
    ...
    if (updateId) {
      ...
      if (
        ...
        (!options?.bypassPassengerCheck &&
          !userCanAccessFlight(flight, user.id))
      ) {
        return {
          success: false,
          type: 'httpError',
...
export const createManyFlights = async (
  ...
  if (!dedupe) {
    ...
    await createManyFlightsPrimitive(
      db,
      data,
  ...
  const userFlights = new Set<number>();
  ...
  for (const flight of existingFlights) {
    ...
    if (userCanAccessFlight(flight, userId)) {
      userFlights.add(flight.id);
    }
  }
  ...
  for (const f of uniqueFlights) {
    ...
    if (existingFlight) {
      ...
      if (mode === 'personal' && userFlights.has(existingId)) {
        continue;
      }
