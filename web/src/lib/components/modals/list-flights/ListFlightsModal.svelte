<script lang="ts">
  ...
  import {
    matchesFlightAudience,
    type FlightAudience,
  } from '$lib/utils/flight-audience';
  ...
  let flightAudience = $state<FlightAudience>('personal');
  const audienceFilteredFlights = $derived(
    filteredFlights.filter((flight) =>
      matchesFlightAudience(flight, flightAudience),
    ),
  );

  const formattedFlights = $derived.by(() => {
    const data = audienceFilteredFlights;
    if (!data) return [];
  ...
  const showFlightDetails = (flight: FlightData) => {
    onNavigate?.({
      destination: 'map',
  ...
  const activateFlight = (flight: FlightData) => {
    if (!readonly && selecting) {
      selectedFlights = selectedFlights.includes(flight.id)
        ? selectedFlights.filter((id) => id !== flight.id)
        : [...selectedFlights, flight.id];
      return;
    }
    if (onNavigate) showFlightDetails(flight);
  };
  ...
  <div>
    <div class="flex flex-col gap-2 max-md:px-4 max-md:py-4">
      ...
      {:else}
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h2 class="text-3xl font-bold tracking-tight">
            {flightAudience === 'guests' ? 'Guest Flights' : 'My Flights'}
          </h2>
          <Tabs bind:value={flightAudience}>
            <TabsList>
              <TabsTrigger value="personal">My flights</TabsTrigger>
      ...
      {#if filters}
        <Toolbar
          ...
          {showingTo}
          {hasTempFilters}
          numOfFlights={audienceFilteredFlights.length}
          modalOpen={open &&
            !addFlightOpen &&
            !mobileEditOpen &&
            !deleteModalOpen}
    ...
    {:else if !$isMediumScreen}
      <MobileFlightList
        ...
        onOpen={onNavigate ? showFlightDetails : undefined}
        {readonly}
      />
                ...
                {#each group.flights as flight, legIndex (flight.id)}
                  ...
                  >
                    ...
                    <Card
                      ...
                      role={onNavigate || outlined ? 'button' : undefined}
                      tabindex={onNavigate || outlined ? 0 : undefined}
                      level="2"
                      class={cn(
